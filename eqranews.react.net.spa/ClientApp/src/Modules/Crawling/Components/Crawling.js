...
import {
	requestCrawlingSources,
	fetchCrawlingSources,
} from '../Actions/Crawling';
...
class Crawling extends React.Component {
	...
	componentDidMount() {
		this.props.requestCrawlingSources();
	}
	...
	renderSourceList() {
		...
		return this.props.sources.map(Source => {
			return (
				<div className="sources" key={Source.Name}>
					<div>{Source.Name}</div>
					{
						// 	<CrawlingSource
						// 		key={'Source_' + i}
...
const mapStateToProps = state => ({
	sources: state.CrawlingSources.list,
});
