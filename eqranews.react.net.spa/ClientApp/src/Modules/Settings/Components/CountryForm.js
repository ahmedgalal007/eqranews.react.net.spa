...
import { requestCreateCountry, requestUpdateCountry } from '../Actions/Country';
...
export class CountryForm extends Component {
	...
	validate = (fieldValues = this.state) => {
		...
		if ('isoCode' in fieldValues)
			temp.isoCode = fieldValues.isoCode ? '' : 'This field is required.';
	...
	handelSubmit = e => {
		...
		if (this.validate()) {
			...
			if (formData.get('id') == 0) {
				this.props.createCountry(formData);
				// window.alert('inserted.' + data);
			} else {
				console.log('request update', data);
				this.props.updateCountry(formData.get('id'), formData);
...
const mapActionToProps = {
	createCountry: requestCreateCountry,
	updateCountry: requestUpdateCountry,
};

export default connect(
	mapStateToProps,
	mapActionToProps
)(withRouter(CountryForm));
